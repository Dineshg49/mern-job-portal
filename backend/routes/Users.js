...
router.route('/job-created').get(function(req, res) {
    User.findOneAndUpdate({ _id: user_pro }, { $push: { jobs_created: job_pro } },
        (err, updated_data) => {
            if (err) {
...
router.get("/edit-job", function(req, res) {
    Job.findOneAndUpdate({ _id: job_pro }, {
        $set: {
            max_applications: req.query.max_applications,
...
router.get("/delete-job3", function(req, res) {
    ...
    User.updateMany({ $or: [{ jobs_applied: job }, { jobs_shortlisted: job }] }, { $pull: { jobs_applied: job, jobs_shortlisted: job } }, function(err, users) {
        if (err) {
            console.log(err);
        } else {
            res.json(users);
...
router.route('/add-applicant').get(function(req, res) {
    Job.findOneAndUpdate({ _id: req.query._id }, { $push: { curr_applicants: user_pro } },
        (err, updated_data) => {
            if (err) {
                console.log("update not done");
            } else {
...
router.route('/apply-job').get(function(req, res) {
    ...
    User.findOneAndUpdate({ _id: user_pro }, { $push: { jobs_applied: req.query._id, application: val } },
        (err, updated_data) => {
            if (err) {
                console.log("update not done");
            } else {
...
router.route('/shortlist-job').get(function(req, res) {
    User.findOneAndUpdate({ _id: req.query._id }, { $push: { jobs_shortlisted: job_pro } },
        (err, updated_data) => {
            if (err) {
                console.log("update not done");
            } else {
...
router.route('/accept-job').get(function(req, res) {
    ...
    User.findOneAndUpdate({ _id: req.query._id }, {
...
router.route('/reject-others').get(function(req, res) {
    ...
    Job.findOneAndUpdate({ $or: [{ curr_applicants: req.query._id }, { curr_shortlisted: req.query._id }] }, {
...
router.route('/rate-job').get(function(req, res) {
    ...
    console.log(x);
    Job.findOneAndUpdate({ _id: req.query.jobid }, {
